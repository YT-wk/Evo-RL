...
def _make_gravity_mock() -> MagicMock:
    gravity = MagicMock()
    gravity.urdf_path = "/tmp/rebot.urdf"
    gravity.torque.return_value = {
        name: 1.25
...
def follower():
    ...
    with (
        ...
        patch(f"{_MODULE}.B601GravityFeedforward", return_value=_make_gravity_mock()),
    ):
...
def test_partial_home_action_is_rejected_before_connect():
    with (
        ...
        RebotB601Follower(
            RebotB601FollowerRobotConfig(port="/dev/null", home_action={"shoulder_pan": 0.0})
        )
...
def test_all_mit_actions_include_gravity_feedforward(follower):
    class FakeGravity:
        urdf_path = "/tmp/rebot.urdf"

        def torque(self, _positions):
            return {name: 1.25 for name in follower.motor_names[:-1]}
    ...
    follower.motors["shoulder_pan"].send_mit.reset_mock()
    follower.send_action({"shoulder_pan.pos": 1.0})
    assert follower.motors["shoulder_pan"].send_mit.call_args.args[-1] == pytest.approx(1.25)


def test_mit_action_rejects_invalid_arm_feedback(follower):
    motor = follower.motors["elbow_flex"]
    original_state = motor.get_state.return_value
    motor.get_state.return_value = None

    try:
        with pytest.raises(RuntimeError, match="invalid feedback for 'elbow_flex'"):
            follower.send_action({"shoulder_pan.pos": 1.0})
    finally:
...
def test_mit_action_rejects_incomplete_gravity_vector(follower):
    original_torque = follower._gravity_feedforward.torque.return_value
    follower._gravity_feedforward.torque.return_value = {"shoulder_pan": 1.25}

    try:
        with pytest.raises(RuntimeError, match="invalid gravity feedforward"):
            follower.send_action({"shoulder_pan.pos": 1.0})
    finally:
        follower._gravity_feedforward.torque.return_value = original_torque
...
def test_safe_home_starts_from_latest_measured_pose_not_last_policy_target(follower):
    ...
    follower.config.home_action = {name: 0.0 for name in last_target}
    follower._startup_home_action = {}
...
def test_bimanual_prefixes_features():
    ...
    assert robot.left_arm.calibration_fpath != robot.right_arm.calibration_fpath
...
def test_bimanual_send_action_runs_both_arms_concurrently():
    barrier = threading.Barrier(2, timeout=1.0)
    ...
    sent = robot.send_action({"left_shoulder_pan.pos": 1.0, "right_shoulder_pan.pos": 2.0})

    assert sent == {"left_shoulder_pan.pos": 1.0, "right_shoulder_pan.pos": 2.0}


def test_bimanual_guarded_takeover_toggles_both_gravity_compensators():
    class FakeArm:
        def __init__(self):
            self.set_gravity_compensation = MagicMock()
    ...
    robot.left_arm.set_gravity_compensation.assert_called_once_with(True)
    robot.right_arm.set_gravity_compensation.assert_called_once_with(True)
...
def test_bimanual_connect_failure_keeps_the_other_arm_enabled_for_manual_recovery():
    class FakeArm:
    ...
    with pytest.raises(RuntimeError, match="right: right arm failed"):
        robot.connect(calibrate=False)

    robot.left_arm.emergency_disable.assert_not_called()
    robot.left_arm.disconnect.assert_not_called()
    robot.right_arm.emergency_disable.assert_not_called()
    robot.right_arm.disconnect.assert_not_called()
