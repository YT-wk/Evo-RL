...
class RebotB601Follower(Robot):
    ...
    def __init__(self, config: RebotB601FollowerRobotConfig):
        ...
        self._last_commanded_arm_positions: dict[str, float] = {}
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        self._last_commanded_arm_positions = {}
        ...
        try:
            ...
            if self.config.control_mode == "mit":
                ...
                logger.info("Using B601-DM gravity feedforward from %s", self._gravity_feedforward.urdf_path)
    ...
    def _present_pos(self, *, require_valid_arm_feedback: bool = False) -> dict[str, float]:
        """Read present joint positions in degrees.

        MIT commands use measured arm positions to calculate gravity feedforward.
        In that path stale or missing feedback is unsafe: reject the command rather
        than manufacturing a zero-position gravity vector.
        """
        if self.bus is None:
            raise DeviceNotConnectedError(f"{self.__class__.__name__} motor bus is not connected.")
        for motor in self.motors.values():
        ...
        try:
            ...
            if require_valid_arm_feedback:
                raise RuntimeError("B601-DM MIT command rejected: CAN feedback poll failed") from error

        present_pos = {}
        for motor_name, motor in self.motors.items():
            state = motor.get_state()
            if state is None or not math.isfinite(state.pos):
                if require_valid_arm_feedback and motor_name != GRIPPER_MOTOR:
                    raise RuntimeError(
                        f"B601-DM MIT command rejected: invalid feedback for '{motor_name}'"
                    )
    ...
    def _log_safe_home_debug_command(
        ...
        for motor_name, position_deg in goal_positions.items():
            ...
            commands[motor_name] = {
                ...
                "tau_ff": "measured g(q) at send time" if motor_name != GRIPPER_MOTOR and use_mit else 0.0,
            }
        logger.info(
    ...
    def _log_safe_home_debug_feedback(
        ...
        feedback_minus_target = {
            name: feedback[name] - target for name, target in target_positions.items()
        }
        logger.info(
    ...
    def _send_goal_positions(
        ...
        if use_mit:
            if self._gravity_feedforward is None:
                raise RuntimeError("B601-DM gravity feedforward is unavailable for MIT control")
            gravity_torque = self._gravity_feedforward.torque(
                self._present_pos(require_valid_arm_feedback=True)
            )
            arm_joints = self._arm_joint_names()
            missing = [name for name in arm_joints if name not in gravity_torque]
            invalid = [
                name
                for name in arm_joints
                if name in gravity_torque and not math.isfinite(float(gravity_torque[name]))
            ]
            if missing or invalid:
                raise RuntimeError(
                    "B601-DM MIT command rejected: invalid gravity feedforward "
                    f"(missing={missing}, non_finite={invalid})"
                )
        for motor_name, position_deg in goal_pos.items():
            ...
            if motor_name == GRIPPER_MOTOR:
                ...
                kp = self.config.mit_kp[idx] if isinstance(self.config.mit_kp, list) else self.config.mit_kp
                kd = self.config.mit_kd[idx] if isinstance(self.config.mit_kd, list) else self.config.mit_kd
                motor.send_mit(pos_rad, 0.0, kp, kd, float(gravity_torque[motor_name]))
            else:
    ...
    def set_gravity_compensation(self, enabled: bool) -> None:
        """Compatibility hook for guarded takeover callers.

        MIT control always uses measured-state gravity feedforward; this flag is
        retained only because existing handoff code invokes it.
        """
        if self.config.control_mode != "mit" and enabled:
            logger.warning("Ignoring B601 gravity compensation request outside MIT control mode.")
        elif self.config.control_mode == "mit" and enabled:
            logger.debug("B601 MIT gravity feedforward is always active.")
    ...
    def _hold_arm_positions_during_gripper_motion(
        ...
        *,
        ...
        while True:
            ...
            now = time.monotonic()
            if (
                debug_started_at is not None
                and now >= next_debug_feedback_at
            ):
        ...
        try:
            ...
            for stage_index, (active_joints, hold_joints) in enumerate(stages, start=1):
                if not active_joints:
                    continue
                frames = self._safe_home_stage_frames(
                    trajectory_start, home, active_joints, total_delta_deg
                )
                logger.info(
                ...
                for frame in range(1, frames + 1):
                    ...
                    positions.update(
                        {
                            name: trajectory_start[name]
                            + (home[name] - trajectory_start[name]) * ratio
    ...
    def send_action(self, action: RobotAction) -> RobotAction:
        ...
        self._last_commanded_arm_positions.update(
            {name: float(value) for name, value in goal_pos.items() if name != GRIPPER_MOTOR}
        )
