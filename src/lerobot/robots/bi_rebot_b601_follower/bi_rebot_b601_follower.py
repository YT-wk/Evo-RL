...
class BiRebotB601Follower(Robot):
    ...
    def calibrate(self) -> None:
        # Calibration is intentionally sequential because each arm requires an
        # operator prompt and a distinct physical zeroing pose.
    ...
    def _run_both(self, operation: str, left_fn, right_fn):
        """Run both arms without changing either arm's torque state on failure."""
        with ThreadPoolExecutor(max_workers=2) as executor:
            futures = {
                "left": executor.submit(left_fn),
                "right": executor.submit(right_fn),
            }
            results = {}
            errors = []
        ...
        if errors:
            ...
            raise RuntimeError(
                f"Bimanual B601 {operation} failed ({details}). "
                "Any successfully connected arm remains enabled for manual recovery."
            ) from errors[0][1]
    ...
    def safe_home(self) -> bool:
        ...
        left_result, right_result = self._run_both(
            "safe-home", self.left_arm.safe_home, self.right_arm.safe_home
        )
        return bool(left_result) and bool(right_result)

    def set_gravity_compensation(self, enabled: bool) -> None:
        """Apply guarded human-takeover gravity compensation to both followers."""
        self.left_arm.set_gravity_compensation(enabled)
        self.right_arm.set_gravity_compensation(enabled)
    ...
    def disconnect(self) -> None:
        ...
        if arms_requiring_home:
            if len(arms_requiring_home) == 2:
                results = list(
                    self._run_both("safe-home", self.left_arm.safe_home, self.right_arm.safe_home)
                )
            else:
                results = [arms_requiring_home[0].safe_home()]
            if not all(results):
                raise RuntimeError(
